...
l <- get_links("electric/lightncon.html",heading="Acceleration")
l
...
$$ f_{in} \simeq  d_{in}^{-1.365} $$ where $f_{in}$ is the frequency and $d_{in}$ is the in-degree
$$ f_{out} \simeq  d_{out}^{-1.552} $$ where $f_{out}$ is the frequency and $d_{out}$ is the out-degree
...
get_sub_tree is a function to sample a graph from the hierarchy where, given a staring node, we sample $n$ nodes one level lower and continue on till the given number of levels. We also define two helper functions to wrap the titles of the nodes and create a layout for a cleaner visualization.  
...
![Attraction Basin](attraction_basin.png)
